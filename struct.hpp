...
struct show_t{
    bool S, gr, fr, lr, cr, ctr, m, M, cache, F, B, label, Point, help, Notify;
    bool next;
    int wid;
    long long Sval;
    int laddr;
    ...
    show_t():S(0), gr(0), fr(0), lr(0), cr(0), ctr(0), m(0), M(0), cache(0),F(0), B(0), label(0), Point(0), help(0), Notify(0), next(1), wid(3), Sval(0), bpoint("", 0){}
};
using SHOW = show_t;
...
struct memory2_t : MEMORY{
    ...
    int32_t notify;
    memory2_t():FL(INSTR_SIZE), notify(-1){}
};
